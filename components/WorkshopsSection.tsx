'use client';

import React from 'react';

const WorkshopsSection = () => {
  return (
    <section className="py-20 sm:py-32 px-4 sm:px-6 lg:px-8 relative" style={{ background: '#ffffff' }}>
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 sm:gap-16 items-center">
          {/* Left Content */}
          <div className="space-y-8">
            <h2 className="font-serif font-bold" style={{ fontSize: 'clamp(2rem, 8vw, 4rem)', color: '#000000', whiteSpace: 'pre-wrap' }}>
              Workshops
              <br />
              <span style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 50%, #a855f7 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                & Training
              </span>
            </h2>

            <p style={{ color: '#666666', fontSize: '16px', lineHeight: '1.8', maxWidth: '500px' }}>
              Result-oriented workshops for business owners who want to generate real results using digital platforms — not just theory.
            </p>

            {/* Workshop list */}
            <div className="space-y-4">
              {[
                { title: 'Meta Ads Mastery', desc: 'Create, launch, and optimize high-converting campaigns.' },
                { title: 'Social Media Management', desc: 'Manage Instagram and Facebook pages strategically.' },
                { title: 'Google My Business', desc: 'Rank locally and attract genuine customers.' },
                { title: 'Content Creation', desc: 'Create professional, sales-driven content.' },
              ].map((item) => (
                <div key={item.title} className="pb-3" style={{ borderBottom: '1px solid #e5e7eb' }}>
                  <h3 style={{ color: '#000000', fontSize: '15px', fontWeight: '600', marginBottom: '4px' }}>
                    {item.title}
                  </h3>
                  <p style={{ color: '#666666', fontSize: '13px' }}>
                    {item.desc}
                  </p>
                </div>
              ))}
            </div>

            {/* CTA Button */}
            <button
              className="group inline-flex items-center gap-3 px-8 py-3 transition-all duration-300 hover:scale-105 mt-4"
              style={{
                border: '2px solid #000000',
                borderRadius: '50px',
                color: '#000000',
                backgroundColor: '#ffffff',
                fontSize: '14px',
                fontWeight: '600',
              }}
            >
              Join a Workshop
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="group-hover:translate-x-1 transition-transform">
                <path d="M1 8h14m-6-6l6 6-6 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>

          {/* Right — Simple accent visual */}
          <div className="hidden lg:flex items-center justify-center">
            <div className="relative w-full h-80">
              {/* Subtle gradient boxes */}
              <div
                className="absolute top-0 right-0 w-48 h-48"
                style={{
                  background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(168, 85, 247, 0.05) 100%)',
                  borderRadius: '30px',
                }}
              />
              <div
                className="absolute bottom-0 left-0 w-32 h-32 rounded-full"
                style={{
                  background: 'radial-gradient(circle, rgba(16, 185, 129, 0.12) 0%, transparent 70%)',
                }}
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default WorkshopsSection;
